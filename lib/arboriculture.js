...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function iterizeForOf(node, path) {
        ...
        var index, iterator, 
            initIterator = parser.part("[$0[Symbol.$1]()]",[node.right,ident(node.await ? "asyncIterator":"iterator")]).expr;
        ...
        node.test = parser.part(
            (node.await ? {
                ...
                code:"!($0[1] = await $0[0].next()).done && (($1 = $0[1].value) || true)"
            } :
    ...
    function asyncDefine(ast) {
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (examine(node).isAsync && examine(node).isFunction) {
                if (node.generator) {
                    function splitYieldAfterAwait(src) {
                        var dest = [] ;
                        var seenAwait = false ;
                        for (var i=0; i<src.length; i++) {
                            var ctnAwait = containsAwait(src[i]) ; 
                            var ctnYield = containsYield(src[i]) ; 
                            if (ctnAwait && ctnYield) {
                                var ex = new SyntaxError(where(src[1]) + 
                                    "Nodent AsyncGenerators cannot contain 'yield' and 'await' in the same expresson. "+
                                    "You can avoid this by using an intermediate value to place the expressions in separate statements",
                                    pr.filename, node.start);
                                ex.pos = src[i].start;
                                ex.loc = src[1].loc.start;
                                throw ex;
                            }
                            if (seenAwait && ctnYield) {
                                dest.push(parser.part("return function*(){ $:afterAwait }",{
                                    afterAwait: splitYieldAfterAwait(src.slice(i))
                                }).body[0]) ;
                            } else {
                                if (ctnAwait)
                                    seenAwait = true ;
                    ...
                    var body = splitYieldAfterAwait(node.body.body) ;
                    var runtime = parser.part({
                        ...
                        noNodentExtensions:false,
                        code: "(async function(y) { var h = g.next(y); if (h.value && typeof h.value.then==='function') { g = (await h.value)(); return g.next(y); } return h; })"
                    }).expr)}).expr ;
                    ...
                    node.body = {
                        type:'BlockStatement',
                        body:parser.part("return $runtime(function *(){$:body})",{
                            runtime:runtime,
                            body:body
                        }).body
                    }
