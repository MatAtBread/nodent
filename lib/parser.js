...
function walkBlock(node, st, c) {
    var i = 0 ;
    var walked = [] ;
    while (true) {
        console.log(node.type,printNode(node.body).replace(/(?:\r\n|\r|\n)/g,"|"));            
        console.log(node.type,node.body.length,i);            
        var n = node.body[i] ;
        walked.push(n) ;
        c(n, st, "Statement");
        var j=0; 
        while (true) {
            if (walked.indexOf(node.body[j])<0) {
                i = j ;
                break ;
            }
            j += 1 ;
            if (j>=node.body.length)
                return ;
        }
    }
}
...
require('acorn-nodent-plugin')(acorn) ;
