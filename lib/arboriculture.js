		...
		parser.treeWalker(ast,function(node,descend,path) {
			if (node.type==='LogicalExpression' && containsAwait(node.right)) {
				if (node.right.type==='LogicalExpression' && node.right.operator===node.operator) {
					...
					console.log("<<",printNode(node.left),">>",printNode(node.right));
				}
				var stmt,body ;
				...
				callBack.body = callBack.body.map(function(n){
					return parser.treeWalker(n,function(child,descend,path){
						if (child===node) {
							path[0].replace(awaited) ;
						} else {
							descend() ;
						}
					}) ;
	...
	function asyncAwait(ast,inAsync,parentCatcher) {
		parser.treeWalker(ast,function(node, descend, path){
			...
			if (examine(node).isAwait) {
				...
				// reference to 'fn', and don't wrap it using $asyncbind (as it will have
				// been bound already)
				if (stmt.self.type==='ReturnStatement' 
					...
					&& stmt.self.argument.callee.type === 'Identifier') {
//					console.log(">>>>",printNode(stmt.self),"<<<<<<<<")	
