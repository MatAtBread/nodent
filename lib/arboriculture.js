  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function labelTryCatch(ast) {
        parser.treeWalker(ast, function (node, descend, path) {
            if (node.type === 'TryStatement') {
                ...
                if ((node.$containedAwait = !!containsAwait(node)) || 
                    (node.finalizer && inAsync(path) && (contains(node.finalizer.body,isExitStatement)))) {
                    node.$needsMapping = true ;
    ...
    function mapTryCatch(node, path, down) {
        if (node.$needsMapping) {
            ...
            if (node.finalizer) {
                var finalizer = parser.part(
                    "var $decl = (function ($exit) {                           "+
                    "   return (function ($value) {                            "+
                    "       {$:body}                                           "+
                    "       return $exit && ($exit.call(this, $value));        "+
                    "   }).$asyncbind(this, $error);                           "+
                    "}).$asyncbind(this);", {
                        decl:ident(node.$seh + "Finally"),
    ...
    function mapLoops(node, path, down) {
        ...
        if (!node.$mapped && examine(node).isLoop && contains(node,mappableLoop)) {
          ...
          if (opts.noRuntime) {
            invokeIterate = parser.part("return ($loop_trampoline = (function (q) { "+
                "    $$setMapped: while (q) { "+
                "         if (q.then)  "+
                "             return q.then($loop_trampoline, $error); "+
                "         try { "+
                "             if (q.pop)  "+
                "                 if (q.length)  "+
                "                 return q.pop() ? $loop_exit.call(this) : q; "+
                "              else  "+
                "                 q = $loop_step; "+
                "              else  "+
                "                 q = q.call(this) "+
                ...
                "}).bind(this))($loop)",{
                     ...
                     loop_trampoline:idTrampoline,
                     error:exit,
                     loop:idIter,
                     loop_exit:idContinuation,
                     loop_step:idStep
                }).body[0] ;
          } else {
            invokeIterate = parser.part("return Function.$0.trampoline(this,$1,$2,$3)($4)",[
                genIdent.asyncbind,                                        
