 ...
 */
function ___contains(ast, fn, descendOn) {
    if (!ast) 
        return null;
    if (fn && typeof fn === 'object') {
        var keys = Object.keys(fn);
...
function asynchronize(pr, __sourceMapping, opts, logger) {
   ...
   */
    ...
    function mapBlock(ast) {
        return ast ;
        parser.treeWalker(ast, function (node, descend, path) {
            if (examine(node).isBlockStatement && examine(path[0].parent).isBlockStatement) {
                var ref = path[0];
                if (containsAwait(node)/* && containsBlockScopedDeclarations(node.body)*/) {
                    var symName = generateSymbol(node);
                    var synthBlock = cloneNode(node) ;
                    if ('index' in ref) {
                        var idx = ref.index;
                        ...
                        ref.parent[ref.field].splice(idx,1,synthBlock,ctn);
//                        ref.replace([synthBlock,ctn])
                    } else {
                        ref.parent[ref.field] = synthBlock;
                    }
                } else {
//                    ref.replace(node.body) ;
    ...
    function hoistDeclarations(ast) {
        function isFreeVariable(kinds) {
            return function(n, path) {
                if (n.type === 'VariableDeclaration' && path[0].parent.type != 'ForStatement' && kinds.indexOf(n.kind)>=0) {
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (examine(node).isBlockStatement) {
                ...
                if (await) {
                    ...
                    var isScope = !path[0].parent || examine(path[0].self).isScope ;            
                    ...
                    if (isScope) {
                        ...
                        consts.forEach(function(d){
                            ...
                            d[0].append({
                                type:'VariableDeclaration',
                                kind:'let',
                                declarations: ids.filter(function(name){ return name in duplicates }).map(function(name){
                                    return {
                                        type:'VariableDeclarator',
                                        id:ident(name)
                                    }
                            ...
                            d[0].self.kind = 'var' ;
                            d[0].self.declarations = ids.filter(function(name){ return name in names }).map(function(name){
                                return {
                                    type:'VariableDeclarator',
                                    id:ident(name)
                                }
                    ...
                    var functions = scopedNodes(node, isHoistableFunction, inStrictBody);
                    
                    vars = vars?extractVars(vars,'var'):{duplicates:{},decls:[]} ;
                    lets = extractVars(lets,'let') ;
