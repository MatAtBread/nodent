...
function asynchronize(pr, __sourceMapping, opts, logger) {
    ...
    function replaceArguments(ast) {
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            } else if (node === ast || node.type === 'ArrowFunctionExpression' || !examine(node).isFunction) {
              if (node.type === 'ArrowFunctionExpression') nafe ++ ;
              descend();
              if (node.type === 'ArrowFunctionExpression') nafe -- ;
            }
        });
//        console.log(ast.type, r, printNode(ast));
        return r;
    }
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function hoistDeclarations(ast) {
      var sequences = {
        ...
        function containsAssignedAwait(ast){
          return contains(ast, function(n){
              return n.type==='AssignmentExpression' && containsAwait(n)
          });
        }
        ...
        parser.treeWalker(ast, function (node, descend, path) {
          ...
          if (node.type in sequences && !node.$hoisted) {
            var expr = node[sequences[node.type]] ;
            var assignments = [] ;
            var stmts = [] ;
            for (i=0; i<expr.length;i++) {
              if (!containsAssignedAwait(expr.slice(i+1)))
                break ;
              if (expr[i].type === 'AssignmentExpression') {
                if (containsAwait(expr[i].right)) {
            ...
            for (i=0;i<path.length;i++) {
              if (examine(path[i].parent).isBlockStatement) {
                [].splice.apply(path[i].parent.body,[path[i].index,0].concat(stmts))
              }
            }
