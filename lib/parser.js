...
var referencePrototypes = {
    ...
    append: function(newNode,prepend,expand) {
        if (Array.isArray(newNode) && newNode.length===1) newNode = newNode[0] ;
        if ('index' in this) {
            if (Array.isArray(newNode)) {
                [].splice.apply(this.parent[this.field],[this.index+(prepend?0:1),0].concat(newNode)) ;
            } else {
                this.parent[this.field].splice(this.index+(prepend?0:1),0,newNode) ;
            }
        } else {
            if (expand) {
                var thisNode = this.parent[this.field][this.index] ;
                if (thisNode.type.match(/[a-zA-Z]+(Declaration|Statement)/)) {
                    thisNode = {
                        type:'BlockStatement',
                        body: Array.isArray(newNode)
                            ? (prepend ? newNode.concat([thisNode]) : [thisNode].concat(newNode))
                            : (prepend ? [newNode,thisNode] : [thisNode,newNode])
                    }
                    return this.self ;
                }
            }
...
var parseCache = {} ;
function partialParse(opts,args) {
  try {
      var code ;
      ...
      if (!parseCache[code]) {
          parseCache[code] = acornParse(code,opts) ;
      }
      ...
      console.error(ex) ;
      throw new Error("Nodent: Internal compilation error\n"+code+"\n"+JSON.stringify(args)) ;
  }
