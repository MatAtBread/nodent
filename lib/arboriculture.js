    ...
    function asyncLoops(ast) {
      parser.treeWalker(ast, function (node, descend, path) {
        ...
        if (examine(node).isLoop && containsAwait(node)) {
          ...
          var label = "LOOP" ;
          var idTrampoline = ident(opts.generatedSymbolPrefix + (label + "_trampoline")) ;
          var idBounce = ident(opts.generatedSymbolPrefix + (label + "_bounce")) ;
          var idIter = ident(opts.generatedSymbolPrefix + (label + "_iter")) ;
          var idContinuation  = ident(opts.generatedSymbolPrefix + (label + "_ctn")) ;
