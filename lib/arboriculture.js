	...
	function mapTryCatch(node, path, down) {
		if (node.type==='TryStatement' && containsAwait(node) && !node.$mapped) {
			...
			if (node.finalizer) { /** TODO: Not yet working! **/
				logger(where(node.finalizer)+"Warning: finally {...} not implemented correctly");
			}
		}
	}
		...
		parser.treeWalker(ast,function(node, descend, path){
			...
			if (examine(node).isAwait) {
				...
				if (!inAsync || inAsync==="warn") {
					var errMsg = where(node)+"Warning: 'await' used inside non-async function. " ;
					if (opts.promises)
				...
				if ((parent.type==='LogicalExpression') && (parent.operator=="||" || parent.operator=="&&") && parent.right===node) {
					logger(where(node.argument)+"Warning: '"+printNode(parent)+"' on right of "+parent.operator+" will always evaluate '"+printNode(node.argument)+"'") ;
				}
				if ((parent.type==='ConditionalExpression') && parent.test!==node) {
					logger(where(node.argument)+"Warning: '"+printNode(parent)+"' will always evaluate '"+printNode(node.argument)+"'") ;
				}
		...
		function warnAsyncExit(exit,fn) {
			if (!fn.$asyncexitwarninig) {
				...
				logger(where(exit)+"'async "
						...
						+"' in generator mode - using Promises for function at "+where(fn)) ;
			}
		}
