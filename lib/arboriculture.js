...
//      isFunctionOrMember:node.type==='FunctionDeclaration' || node.type==='FunctionExpression' || node.type==='Function' || node.type==='ArrowFunctionExpression'
//          || node.type==='ObjectMethod' || node.type==='ClassMethod',
        ...
        isAsync:node.async,
        isStatement:node.type.match(/[a-zA-Z]+Declaration/)!==null || node.type.match(/[a-zA-Z]+Statement/)!==null,
...
function asynchronize(pr,__sourceMapping,opts,logger) {
    ...
    function getAsyncMember(node) {
        if (opts.babelTree && (node.type==='ClassMethod' || node.type==='ObjectMethod') && node.async) {
            return node ;
        } else if (((opts.babelTree && (node.type==='ClassProperty' || node.type==='ObjectProperty'))
            ...
            if (examine(node).isLoop && containsAwait(node)) {
                ...
                if (init) {
                    if (!examine(init).isStatement)
                        ...
                        var decl = init ;
                        var init = {type:'ExpressionStatement',expression:{
                            type:'SequenceExpression',
                            expressions:init.declarations.map(function(d){
                                return {
                        ...
                        decl.declarations.forEach(function(d){ delete d.init }) ;
                        for (var p=0; p<path.length;p++) {
                            if (examine(path[p].parent).isScope) {
                                if (path[p].parent.body.body[0].type === 'VariableDeclaration')
                                    path[p].parent.body.body[0].declarations =
                                        path[p].parent.body.body[0].declarations.concat(decl.declarations) ;
                                else
                                    path[p].parent.body.body.unshift(decl) ;
                                break ;
                            }
                        }
                    }
...
debugger;
        return parser.treeWalker(ast,function(node,descend,path){
            descend();
            var transform = getAsyncMember(node) ;
            if (!transform)
                return ;
            ...
            if (!containsAsyncExit(transform) && 
                (transform.body.body.length===0 || transform.body.body[transform.body.body.length-1].type !== 'ReturnStatement')) {
                transform.body.body.push({type:'ReturnStatement'}) ;
            }
        ...
        parser.treeWalker(ast,function(node, descend, path){
            ...
            if (examine(node).isAsync && examine(node).isFunction) {
                ...
                if (examine(node.body).isBlockStatement) {
                    if (!containsAsyncExit(node.body) && 
                        (node.body.body.length===0 || node.body.body[node.body.body.length-1].type !== 'ReturnStatement')) {
                        node.body.body.push({type:'ReturnStatement'}) ;
                    }
                    fnBody = {
                        type:'BlockStatement',
                        body:node.body.body.map(function(sub){ return mapReturns(sub,path)})
                    } ;
                } else {
                    fnBody = {
                            type:'BlockStatement',
                            body:[mapReturns({type:'ReturnStatement',argument:node.body},path)]
                    } ;
                ...
                fnBody = {type:'BlockStatement',body:[{
                    type:'ReturnStatement',
                    loc:node.loc,
                    argument:fnBody
                }]};
  ...
  function spawnBody(body, deferExit) {
    ...
    function asyncSpawn(ast) {
        function warnAsyncExit(exit,fn) {
            if (!fn.$asyncexitwarninig) {
                ...
                logger(where(exit)+"'async "
                        ...
                        +"' not possible in generator mode. Using Promises for function at "+where(fn)) ;
            }
        }
        ...
        parser.treeWalker(ast,function(node, descend, path){
            ...
            } else if (fn = getAsyncMember(node)) {
