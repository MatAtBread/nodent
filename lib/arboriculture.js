...
function examine(node) {
    ...
    return {
        getScope:(node.type==='FunctionDeclaration' || node.type==='FunctionExpression' || node.type==='Function'
            || node.type==='ObjectMethod' || node.type==='ClassMethod'
            || (node.type==='ArrowFunctionExpression' && node.body.type==='BlockStatement'))?node.body.body
...
function asynchronize(pr,__sourceMapping,opts,logger) {
    ...
    function getMemberFunction(node) {
        if (opts.babelTree && (node.type==='ClassMethod' || node.type==='ObjectMethod')) {
            return node ;
        } else if (((!opts.babelTree && (node.type==='MethodDefinition'))
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function transformForIn(node,path) {
        ...
        var it = node.left.type==='VariableDeclaration'
        ...
        var parent = path[0].parent ;
        if ('index' in path[0]) {
            parent.body.splice(path[0].index,1,init,node,loop) ;
        } else {
            parent[path[0].ref] = { type: 'BlockStatement', body:[init,node,loop]} ;
        }
    }
                ...
                if (init) {
                    if (!examine(init).isStatement)
                        ...
                        var init = {type:'ExpressionStatement',expression:{
                            ...
                            expressions:init.declarations.filter(function(d){return d.init}).map(function(d){
                                return {
                                    type:'AssignmentExpression',
                                    operator:'=',
                                    left:d.id,
                        ...
                        for (var p=0; p<path.length;p++) {
                            ...
                            if (scope = examine(path[p].parent).getScope) {
                                if (scope[0].type === 'VariableDeclaration')
                                    scope[0].declarations =
                                        scope[0].declarations.concat(decl.declarations) ;
                                else
                                    scope.unshift(decl) ;
                                break ;
                            }
