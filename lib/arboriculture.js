...
function asynchronize(pr,__sourceMapping,opts,logger) {
	...
	function getExit(path) {
		for (var n=0;n<path.length;n++) {
			if (path[n].self.$exit) {
				return (path[n].self.$exit) ;
			}
			if (path[n].parent && path[n].parent.$exit) {
				return (path[n].parent.$exit) ;
			}
		}
		return null ;
	}
		...
		return parser.treeWalker(n,function(node,descend,path) {
			...
			if (node.type==='ReturnStatement' && !node.$mapped) {
				...
				if (!(examine(node.argument).isUnaryExpression && node.argument.operator==="void")) {
					node.argument = {
						"type": "CallExpression",
				    	callee:ident((getExit(path)||opts).$return),
				...
				node.argument = {
			    	type:'CallExpression',
			    	callee:ident((getExit(path)||opts).$error),
	...
	function mapTryCatch(node, path, down) {
		if (node.type==='TryStatement' && containsAwait(node) && !node.$mapped) {
			...
			if ('index' in ref) {
				...
				if (afterTry.length) {
					var ctnName = generateSymbol("PostTry") ;
			...
			if (node.handler) {
				var symCatch = ident((getExit(path)||opts).$error) ; 
...
//				var finalizer = makeContinuation(generateSymbol("Finally"),cloneNode(node.finalizer.body)) ;
//				ref.parent[ref.field].unshift(finalizer) ;
//				var callFinally = thisCall(finalizer.id.name) ;
//				node.finalizer.body = [cloneNode(callFinally)] ;
//				delete node.finalizer ;
//				afterContinuation.body.body.unshift(cloneNode(callFinally))
//setExit(afterContinuation,finalizer.id.name) ;				

				afterContinuation.body.body = node.finalizer.body.concat(afterContinuation.body.body) ;
//				logger(where(node.finalizer)+"finally {...} not implemented correctly");
	...
	function asyncAwait(ast,inAsync,parentCatcher) {
		parser.treeWalker(ast,function(node, descend, path){
			...
			if (examine(node).isAwait) {
				...
				cbBody = asyncAwait(cbBody,inAsync,containingExits) ;

				var catcher = ident(containingExits.$error);				
				var returner ;
	...
	function asyncLoops(ast) {
		parser.treeWalker(ast,function(node, descend, path){
			...
			if (node.type==='ForInStatement')
				logger(where(node)+"for...in loops do not asynchonize sequentially - all iterations start in parallel") ;
			if (node.type==='ForOfStatement')
				logger(where(node)+"for...of loops do not asynchonize sequentially - all iterations start in parallel") ;
  ...
  function test(x) {
	...
	function asyncDefineMethod(ast) {
		return parser.treeWalker(ast,function(node,descend,path){
			descend();
			if ((node.type==='MethodDefinition' || (node.type==='Property' && (node.method || node.kind=='get' || node.kind=='set'))) 
					&& node.async && examine(node.value).isFunction) {
				if (node.kind=='set') {
					var ex = new SyntaxError(where(node.value)+"method 'async set' cannot be invoked", pr.filename, node.start) ;
					ex.pos = node.start ;
			...
			if (examine(node).isAsync && examine(node).isFunction) {
				...
				var funcback = {
					...
					callee:{
						...
						property:{
							loc:node.loc,
							type:'Identifier',name:opts.bindAsync},
						computed:false
					}
				} ;
	          ...
	          "argument": {
	            ...
	            "callee": {
	              ...
	              "object": {
	                ...
	                "generator": true,
	                "expression": false,
	                "params": [ident(opts.$return),ident(opts.$error)],
	                "body": {
	                	type:'BlockStatement',
	                	body:mapAsyncReturns(body).concat({
		                	type:'ReturnStatement',
		                	argument:ident(opts.$return)
	                	})
	                 }
	              },
	              "property": {
	...
	function labelTryCatch(ast) {
		parser.treeWalker(ast,function(node, descend,path){
			if (node.type==='TryStatement') {
				...
				if (node.finalizer) {
					setExit(node,{$error:generateSymbol("Finally"),$return:parent.$return}) ;
					if (!node.handler) {
						// We have a finally, but no 'catch'. Create the default catch clause 'catch(_ex) { throw _ex }'
						var exSym = generateSymbol("exception") ;
						node.handler = {
							"handler": {
								"type": "CatchClause",
								"param": { "type": "Identifier", "name": exSym },
								"body": {
									"type": "BlockStatement",
									"body": [{
										"type": "ThrowStatement",
										"argument": { "type": "Identifier", "name": exSym }
									}]
								}
							}
						};
					}
					setExit(node.handler,parent) ;
					setExit(node.finalizer,parent) ;
				} else {
					if (!node.handler) {
						var ex = new SyntaxError(where(node.value)+"try requires catch or finally clause", pr.filename, node.start) ;
						ex.pos = node.start ;
						ex.loc = node.loc.start ;
						throw ex ;
					}
					// We have a handler by no finally
					setExit(node,{$error:generateSymbol("Finally"),$return:parent.$return}) ;
					setExit(node.handler,parent) ;
				}
			}
