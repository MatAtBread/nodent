    ...
    function asyncDefine(ast) {
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (examine(node).isAsync && examine(node).isFunction) {
                if (node.generator) {
                    function splitYieldAfterAwait(src,dest) {
                        var seenAwait = false ;
                    ...
                    var next = asyncDefine(parser.part({
                        noNodentExtensions:false,
                        code:"(async function() { var h = g.next(); if (h.value && h.value.then) { g = (await h.value)(); return g.next(); } return h; })"
                    }).expr) ;
                    node.async = false ;
                    node.generator = false ;
                    node.body = {
                        type:'BlockStatement',
                        body:parser.part("var g = (function *(){$:body})(); return { [Symbol.asyncIterator]: function(){return this}, next:$next }",{
                            body:body,
                            next:next
                        }).body
                    }
