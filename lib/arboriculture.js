...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  function test(x) {
            ...
            vars.forEach(function (path) {
                ...
                for (var i = 0;i < self.declarations.length; i++) {
                    ...
                    case 'Identifier':
                        ...
                        if (idx >= 0) {
                            logger(where(self.declarations[i]) + "Duplicate 'var " + name);
                        } else {
                ...
                for (var h=0; h<body.length; h++) {
                    if (body[h].type === 'VariableDeclaration') {
                        for (var i=0; i<body[h].declarations.length; i++) {
                            switch (body[h].declarations[i].id.type) {
                            case 'Identifier':
                                names[body[h].declarations[i].id.name] = body[h].kind ;
                                break ;
                            case 'ArrayPattern':
                                body[h].declarations[i].id.elements.forEach(function(e){ names[e.name] = body[h].kind }) ;
                                break ;
                            case 'ObjectPattern':
                                body[h].declarations[i].id.properties.forEach(function(p){ names[p.key.name] = body[h].kind }) ;
                                break ;
                            }
                        }
                    }
    ...
    function hoistDeclarations(ast) {
        function isFreeVariable(kind) {
            return function(n, path) {
                if (n.type === 'VariableDeclaration' && n.kind === kind) {
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            descend();
            if (examine(node).isBlockStatement && containsAwait(node)) {
                ...
                var blockScope = inStrictPath(path) ;
                var vars = extractVars(scopedNodes(node, isFreeVariable('var'),false),'var');
                var lets = extractVars(scopedNodes(node, isFreeVariable('let'),true),'let');
                var consts = extractVars(scopedNodes(node, isFreeVariable('const'),blockScope),blockScope ? 'let':'var');
                 ...
                 * our own block */
                var directives = scopedNodes(node, function (n) { return examine(n).isDirective },true);
                ...
                if (directives.length || vars.length || lets.length || consts.length || functions.length) {
                    node.body = directives.concat(vars).concat(lets).concat(consts).concat(functions).concat(node.body);
                }
            }
            return true;
        });
    ...
    function inStrictPath(path) {
        for (var i=0; i<path.length; i++) {
            if ((path[i].self.type === 'Program' && (path[i].self.sourceType === 'module' || isStrict(path[i].self)))
                || (examine(path[i].self).isFunction && isStrict(path[i].self))) {
                return path[i].self ;
            }
        }
        return null ;
