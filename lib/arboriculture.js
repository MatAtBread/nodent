    ...
    function hoistDeclarations(ast) {
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (examine(node).isBlockStatement && containsAwait(node)) {
                ...
                var vars = extractVars(scopedNodes(node, isFreeVariable('var'),true),'var');
                var lets = extractVars(scopedNodes(node, isFreeVariable('let'),true),'let');
                var consts = scopedNodes(node, isFreeVariable('const'),true) ;

                /* Ensure no consts are on the LHS of an AssignmentExpression
                var constNames = [] ;
                consts.forEach(function (path){
                    constNames = constNames.concat(path[0].self.declarations.map(function(d){ return d.id.name })) ;
                }) ;                
                console.log("NAMES",constNames) ;
                parser.treeWalker(ast,function(n,descend,path){
                    if (n.type==='AssignmentExpression' && constNames.indexOf(n.left.name)>=0)
                        console.log("CONST is assigned: ",printNode(n)) ;
                    else descend() ;
                }) ;
                */
                consts = extractVars(consts,'let');
