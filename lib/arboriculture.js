...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  function test(x) {
            ...
            vars.forEach(function (path) {
                function addName(name) {
                    if (name in definitions) {
                        logger(where(decl.id) + "Duplicate declaration " + name);
                    } else {
                ...
                for (var i = 0;i < self.declarations.length; i++) {
                    ...
                    switch (decl.id.type) {
                    case 'Identifier':
                        addName(decl.id.name);
                        break ;
                    case 'ObjectPattern':
                        decl.id.properties.forEach(function(prop){
                            addName(prop.key.name);
                        }) ;
                        break ;
                    case 'ArrayPattern':
    ...
    function hoistDeclarations(ast) {
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            var prevScope = blockScope ;
            blockScope = blockScope || isStrict(node) ;
            ...
            if (examine(node).isBlockStatement && containsAwait(node)) {
                ...
                var vars = ['var'] ;
                var lets = ['let'] ;
                if (blockScope) lets.push('const') ;
                else vars.push('const') ;
                vars = extractVars(scopedNodes(node, isFreeVariable(vars),false),'var');
                lets = extractVars(scopedNodes(node, isFreeVariable(lets),true),'let');
                ...
                if (directives.length || vars.length || lets.length || functions.length) {
                    node.body = directives.concat(vars).concat(lets).concat(functions).concat(node.body);
                }
            }
