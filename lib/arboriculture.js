          ...
          if (node.type in sequences && !node.$hoisted) {
            ...
            for (i=0; i<expr.length;i++) {
              if (!containsAwait(expr.slice(i+1)))
                break ;
              var assignPath ;
              if (assignPath = containsAssign(expr[i])) {
                var assign = assignPath[0].self ;
                if (containsAwait(assign.right))
                    moveAssignments() ;
                assignments.push(cloneNode(assign)) ;
                coerce(assign,assign.left);
              }
            }
            moveAssignments() ;
            if (stmts.length) for (i=0;i<path.length;i++) {
              var block ;
              if (block = examine(path[i].parent).isBlockStatement) {
                [].splice.apply(block,[path[i].index,0].concat(stmts)) ;
                break ;
              }
            }
