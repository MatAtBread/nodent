...
function containsAwait(ast) {
    return contains(ast, function(n){
        return (n.type==='AwaitExpression' && !n.$hidden) || (n.type==='ForOfStatement' && n.await)
    });
}

function containsYield(ast) {
    return contains(ast, function(n){
        return n.type==='YieldExpression' && !n.$hidden
    });
}
...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
                ...
                if (opts.wrapAwait) {
                    expr = {
                        ...
                        callee: (opts.promises || opts.generators)?{
                            ...
                            parser.part(
                                opts.es6target
                                    ?'(function(v){ return v && typeof v.then==="function" ? v : {then:function(r){return r(v)}}})'
                                    :'(v=>v && typeof v.then==="function" ? v : {then:r=>r(v)})'
                            ).expr
                        :{
                            // ES7 makeThenable
                            type: 'MemberExpression',
    ...
    function iterizeForOf(node, path) {
        ...
        var index, iterator, 
            initIterator = parser.part("[(function(g){return g[Symbol.$1] ? (g[Symbol.$1])():g()})($0)]",[node.right,ident(node.await ? "asyncIterator":"iterator")]).expr;
        ...
        node.test = parser.part(
            (node.await ? {
                noNodentExtensions:false,
                
                code: /*!opts.wrapAwait 
                    ? "!($0[1] = await Promise.resolve($0[0].next())).done && (($1 = $0[1].value) || true)"
                    :*/ "!($0[1] = await $0[0].next()).done && (($1 = $0[1].value) || true)"
            } :
    ...
    function mapLoops(node, path, down) {
        ...
        } else if (node.type === 'ForOfStatement' && (node.await || containsAwait(node))) {
    ...
    function asyncDefine(ast) {
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (examine(node).isAsync && examine(node).isFunction) {
                if (node.generator) {
                    var id = node.id && cloneNode(node.id) ;
                    var body = parser.treeWalker(node.body, function (node, descend, path) {
                        descend() ;
                        var result = ident(generateSymbol("y")) ;
                        if (node.type === 'YieldExpression' && node.delegate) {
                            var yieldStar = parser.part("function*_(){for (var $result,$next,$generator = $sub; !($next = $generator.next()).done; $result = yield $next.value);}",{
                                sub:node.argument,
                                next:ident(generateSymbol("i")),
                                result:result,
                                generator:ident(generateSymbol("g"))
                            }).body[0].body.body ;

                            path[0].replace(result) ;

                            for (var i=0; i<path.length; i++) {
                            	if (examine(path[i].parent).isBlockStatement) {
                            	    path[i].append(yieldStar,true,true) ;
                                    break ;
                            	}
                            }
                    ...
                    var runtime = parser.part({
                        noNodentExtensions:false,
                        code: "(function(g) { return g = g(),Object.create({[Symbol.asyncIterator]: function (){return this}},{next:{value:$next}}) })"
                    },{next:asyncDefine(parser.part({
                        noNodentExtensions:false,
                        code: "(async function(y) { for (var h = g.next(y); (h.value && typeof h.value.then==='function'); (h = g.next(await h.value))) {} return h; })"
                    },{ asyncbind: genIdent.asyncbind}).expr)}).expr ;
                    
                    node.async = false ;
                    node.generator = false ;
                    node.body = {
                        type:'BlockStatement',
                        body:parser.part("return $runtime(function* $ident(){$:body})",{
                            runtime:runtime,
                            ident:id,
                            body:body
                        }).body
