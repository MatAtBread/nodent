...
myfn("ok") ;
		...
		parser.treeWalker(ast,function(node, descend,path){
			if (node.type==='TryStatement' && containsAwait(node)) {
				...
				if (node.finalizer) {
					...
					setExit(node.block,{
						$error:node.$seh+"Catch",//deferredFinally(ident(node.$seh+"Catch")),
						$return:deferredFinally(parent.$return)
					}) ;
	...
	function mapTryCatch(node, path, down) {
		if (node.type==='TryStatement' && containsAwait(node) && !node.$mapped) {
			...
			if (node.handler) {
				var symCatch = ident(node.$seh+"Catch"); //getExit(path,[opts]).$error ; 
				// catcher is not a continuation as it has arguments
				var catchBody = cloneNode(node.handler.body.body) ;
				/*catchBody.unshift({
				...
				var catcher = makeBoundFn(symCatch.name,catchBody,[cloneNode(node.handler.param)],getExit(path,[opts]).$error) ;
			...
			if (node.finalizer) { 
				...
				var callFinally = {type:'ReturnStatement',argument:thisCall({type:'CallExpression',callee:ident(node.$seh+"Finally"),arguments:[ident(node.$seh+"Post")]})} ;
				catchBody[catchBody.length-1] = callFinally ;
