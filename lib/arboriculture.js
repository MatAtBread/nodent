...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function containsAsyncExit(ast, includeNested) {
        try {
            if (Array.isArray(ast)) {
                return ast.some(function(n) { return containsAsyncExit(n, includeNested) });
            }
            ...
            parser.treeWalker(ast, function (node, descend, path) {
                ...
                if (examine(node).isFunction) {
                    var f = contains(node, function (exit) {
                        // Legacy reference to $return or $error - treat as if there is an asyncExit (it might be)
                        // so we behave as previous releases did
                        if (exit.type === 'Identifier' && (exit.name === opts.$return || exit.name === opts.$error)) {
                            throw exit;
                        }
                        if (exit.type === 'ReturnStatement' || exit.type === 'ThrowStatement') {
                            if (examine(exit).isAsync) {
                                throw exit;
                            }
                        }
                    }, includeNested);
