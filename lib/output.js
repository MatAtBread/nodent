...
function formatExpression(code, state, parent, node) {
...
traveler = {
    ...
    ExpressionStatement: function (node, state) {
    	if (node.expression.type==='FunctionExpression') {
        	...
        	this.formatExpression(state.code, state, node, node.expression);
        	state.code.write(null, ')');
    	} else {
    		this.formatExpression(state.code, state, node, node.expression);
    	}
    ...
    ExportNamedDeclaration: function (node, state) {
        ...
        if (node.declaration) {
            ...
            if (specifiers && specifiers.length > 0) {
            	var s = specifiers.map(function(specifier){
                    if (specifier.name) { 
	                    name = specifier.name.local;
	                    code.write(specifier, name);
	                    if (name !== specifier.exported.name) 
	                    	return (specifier.exported + ' as ' + specifier.exported.name);
	                    else
	                    	return name ;
                    }
            	}).join(', ') ;
    ...
    YieldExpression: function (node, state) {
        ...
        if (node.argument) {
            code.write(null, ' ');
            this.formatExpression(code, state, node, node.argument);
        }
    },
    ...
    BinaryExpression: BinaryExpression = function (node, state) {
        ...
        this.formatExpression(code, state, node, node.left);
        code.write(node, ' ', operator, ' ');
        this.formatExpression(code, state, node, node.right);
    },
    ...
    ConditionalExpression: function (node, state) {
        var code = state.code;
        this.formatExpression(code, state, node, node.test);
        code.write(node, ' ? ');
        this.formatExpression(code, state, node, node.consequent);
        code.write(null, ' : ');
        this.formatExpression(code, state, node, node.alternate);
    },
    ...
    CallExpression: function (node, state) {
        var arg;
        var code = state.code;
//if ((precedence(node) <= precedence(node.callee)) != (PARENTHESIS_NEEDED[node.callee.type] === 0))
//		console.log("++++",node.type,node.callee.type);
    ...
    MemberExpression: function (node, state) {
        var code = state.code;
//if ((precedence(node) <= precedence(node.object)) != (PARENTHESIS_NEEDED[node.object.type] === 0))
//	console.log("****",node.type,node.object.type);
