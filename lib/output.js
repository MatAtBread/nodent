...
function precedence(node) {
    var p = OPERATORS_PRECEDENCE[node.type] || OPERATORS_PRECEDENCE[node.operator];
    if (p !== undefined) 
        return p;
...
traveler = {
    ...
    ArrayExpression: ArrayExpression = function (node, state) {
        ...
        if (node.elements.length > 0) {
            ...
            for (var i = 0; ; ) {
                element = elements[i];
                this.expr(state,CommaList,element) ;
                if (++i < length) 
                    state.write(null, ', ');
                 else 
                    break;
    ...
    MemberExpression: function (node, state) {
    	var requireParens = (node.object.type === 'ObjectExpression' || (node.object.type === 'Literal' && node.object.raw.match(/^[0-9]/))) ;
