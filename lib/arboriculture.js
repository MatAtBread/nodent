...
function asynchronize(pr, __sourceMapping, opts, logger) {
    ...
    function asyncTransforms(ast, awaitFlag) {
        ...
        (opts.promises?Nothing:asyncLoops_1)(ast);
        // Handle the various JS control flow keywords by splitting into continuations that could
        // be invoked asynchronously
        mapLogicalOp(ast);
        mapCondOp(ast);
        walkDown(ast, [mapTryCatch,(opts.promises?mapLoops:Nothing),mapIfStmt,mapSwitch,mapBlock]);
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    /* Map loops */
    /* As of v2.7.0 loops are asynchronized via a trampoline to prevent stack overflow in big loops with no actionable awaits. */
    //function asyncLoops_2(ast) {
    //  parser.treeWalker(ast, function (node, descend, path) {
    function mapLoops(node, path, down) {          
        ...
        if (!node.$mapped && examine(node).isLoop && containsAwait(node)) {
          var p ;
          var mapped = [] ;
          var init = node.init;
          var test = node.test;
          ...
          if (path[0].parent.type === 'LabeledStatement') {
            label = path[0].parent.label.name ;
            ref = path[1] ;
          } else {
          ...
          label = opts.generatedSymbolPrefix + node.type.replace(/(Statement|Expression)/g,"") + "_" + label;
          var idTrampoline = ident(label + "_trampoline") ;
          var idBounce = ident("q") ;
          var idIter = ident(label + "_iter") ;
          var idStep = ident(label + "_step") ;
          var idContinuation  = ident(label + "_exit") ;
          var idBreak = idContinuation;//ident(label + "_break") ;

          var continuation ;
          if ('index' in ref) {
            var idx = ref.index;
            var deferredCode = ref.parent[ref.field].splice(idx + 1, ref.parent[ref.field].length - (idx + 1));
            continuation = makeContinuation(idContinuation.name, deferredCode) ;
          }
          ...
          parser.treeWalker(body, function mapExits(n, descend, subpath) {
              if (n.type === 'BreakStatement') {
                var exitLabel = n.label ? ident(opts.generatedSymbolPrefix + node.type.replace(/(Statement|Expression)/g,"") + "_" +n.label.name+"_break") : idBreak ;
                subpath[0].replace({ "type": "ReturnStatement", "argument": exitLabel }) ;
              } else if (n.type === 'ContinueStatement') {
                var exitLabel = n.label ? ident(opts.generatedSymbolPrefix + node.type.replace(/(Statement|Expression)/g,"") + "_" +n.label.name+"_step") : idStep ;
                subpath[0].replace({ "type": "ReturnStatement", "argument": exitLabel }) ;
              } else if (examine(n).isFunction || examine(n).isLoop) {
                return true;
              } else {
                descend();
              }
...
/*          body.unshift(({
            ...
            body:{
              ...
              body:[{
                  type:'ReturnStatement',
                  argument:idContinuation/*{
                      "type": "CallExpression",
                      "callee": {
                          "type": "MemberExpression",
                          "object": idContinuation,
                          "property": ident('call')
                      },
...
*/
          body.unshift(({
            ...
            "body": {
              "type": "BlockStatement",
              "body": step?[ step,returnIter ]:[ returnIter ]
            }
          })) ;
                  ...
                  "body": [{
                      ...
                      "consequent": {
                        ...
                        "argument": {
                          ...
                          "arguments": [ idTrampoline, genIdent.error] // TODO: Check if $error is always the correct context - what about inside an try / catch / finally?
                        }
                      },
                      "alternate": {
                        "type": "ExpressionStatement",
            ...
            if (examine(node).isLoop && containsAwait(node)) {
                ...
                var replace = {
                    ...
                    expression: {
                        ...
                        argument: {
                            type: 'CallExpression',
                            arguments: [{ type: 'ThisExpression' }, literal(0)],
                            callee: {
                                type: 'MemberExpression',
                                object: subCall,
                                property: genIdent.asyncbind,
    ...
    function asyncDefineMethod(ast) {
        return parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (opts.promises) {
                ...
                else if (!opts.promises)
                    funcback.arguments.push(literal(0));
                transform.body = {
                    type: 'BlockStatement',
                    body: [{
                        type: 'ReturnStatement',
            ...
            if (examine(node).isAsync && examine(node).isFunction) {
                ...
                if (opts.promises) {
                    ...
                    if (!opts.lazyThenables) 
                        ...
                        fnBody.arguments.push(literal(0));
                }
                fnBody = {
                    type: 'BlockStatement',
                    body: [{
