		...
		return parser.treeWalker(n,function(node,descend,path) {
			...
			if (node.type==='ReturnStatement' && !node.$mapped) {
				if (lambdaNesting > 0) {
					if (node.async) {
						delete node.async ;
						value = node.argument?[cloneNode(node.argument)]:[] ;
						repl = node ;
					}
				} else {
					if (!containers) {
	...
	function mapTryCatch(node, path, down) {
		if (node.type==='TryStatement' && containsAwait(node) && !node.$mapped) {
			...
			if ('index' in ref) {
				...
				if (afterTry.length) {
					var ctnName = generateSymbol(node) ;
	...
	function asyncAwait(ast,inAsync,parentCatcher) {
		parser.treeWalker(ast,function(node, descend, path){
			...
			if (examine(node).isAwait) {
				...
				var result = {type:'Identifier',name:generateSymbol("await")} ;
	          ...
	          "argument": {
	            ...
	            "callee": {
	              ...
	              "object": {
	                ...
	                "generator": true,
	                "expression": false,
	                "params": [{type:'Identifier',name:opts.$return},{type:'Identifier',name:opts.$error}],
	                "body": {
	                	type:'BlockStatement',
	                	body:mapAsyncReturns(body).concat({
		                	type:'ReturnStatement',
		                	argument:{type:'Identifier',name:opts.$return}
	                	})
	                 }
	              },
	              "property": {
	...
	function labelTryCatch(ast) {
		parser.treeWalker(ast,function(node, descend,path){
			if (node.type==='TryStatement') {
				...
				setCatch(node,generateSymbol(node.handler)) ;
