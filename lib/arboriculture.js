	...
	function asyncDefineMethod(ast) {
		return parser.treeWalker(ast,function(node,descend,path){
            ...
            if (!containsAsyncExit(transform) && 
                (transform.body.length===0 || transform.body[transform.body.length-1].type !== 'ReturnStatement')) {
                transform.body.push({type:'ReturnStatement'}) ;
            }
			...
			var funcback = {
					...
					callee:{
						...
						object:setExit({
							...
							body:mapReturns(transform.body,path),
							$wasAsync:true
						},opts),
						property:genIdent.asyncbind,
						computed:false
