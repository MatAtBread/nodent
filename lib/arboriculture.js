...
function asynchronize(pr, __sourceMapping, opts, logger) {
    ...
    function bindAsync(left,arg) {
        if (opts.noRuntime) {
            if (arg) {
                ...
                var idCatch = ident('$boundEx');
                // Add a global synchronous exception handler to (left)
                var handled = {
                    $mapped: true,
                    type:"TryStatement",
                    block:cloneNode(left.body),
                    handler:{
                        type:'CatchClause',
                        param:idCatch,
                        body:{
        ...
        return {
            ...
            "arguments": Object.freeze(params)
        } ;
    } 
    ...
    var runtimeTrampoline = {
        "type": "MemberExpression",
        "property": ident('trampoline'),
        "computed": false,
        "object": {
            "type": "MemberExpression",
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function mapLoops(node, path, down) {
        ...
        if (!node.$mapped && examine(node).isLoop && containsAwait(node)) {
          ...
          var invokeIterate ;
          if (opts.noRuntime) {
            invokeIterate = inlineTrampoline() ;
          } else {
            invokeIterate = {
              "type": "ReturnStatement",
              "argument": {
                ...
                "callee": {
                  "type": "CallExpression",
                  "callee": runtimeTrampoline,
                  "arguments": [{"type": "ThisExpression"},idContinuation,idStep,genIdent.error]
                },
                "arguments": [idIter]
              }
            };
                                                    ...
                                                    "consequent":{
                                                        ...
                                                        consequent:{
                                                            ...
                                                            argument:{
                                                                ...
                                                                consequent:thisCall(idContinuation),
                                                                alternate:idBounce
                                                            }
                                                        },
                                                        alternate:{
                                            ...
                                            handler:{
                                                ...
                                                body:{
                                                    ...
                                                    body:[{
                                                        ...
                                                        argument:{
                                                            type:'CallExpression',
                                                            callee:genIdent.error,
                                                            arguments:[idCatchParam]
                                                        }
                                                    }]
                                                }
                                            }
    ...
    function asyncLoops_1(ast) {
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            function mapExits(n, descend) {
                if (n.type === 'BreakStatement') {
                    coerce(n, cloneNode(mapBreak(n.label && opts.generatedSymbolPrefix + node.type.replace(/Statement|Expression/g,"") + "_" + n.label.name + "_exit")));
                } else if (n.type === 'ContinueStatement') {
                    coerce(n, cloneNode(mapContinue(n.label && opts.generatedSymbolPrefix + node.type.replace(/Statement|Expression/g,"") + "_" + n.label.name + "_next")));
            ...
            if (examine(node).isLoop && containsAwait(node)) {
                ...
                label = node.type.replace(/Statement|Expression/g,"") + "_" + (label || generatedSymbol++);
