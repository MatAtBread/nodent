...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function transformForIn(node, path) {
        ...
        var pushValues = {
            // for (var idx in <<expr>>) $in_1.push(idx);
            type:'ForInStatement',
            left:idx,
            right:node.right,
            body:{
        ...
        var loop = {
            // for (<<node.left>>; in.length;) { <<ident = $in.shift() ; .... }
            $label:label,
            type: 'ForStatement',
            init: node.left, 
            test: {
            ...
            body: {
                type:'BlockStatement',
                body: node.body.type==='BlockStatement'? [indexAssign].concat(node.body.body):[indexAssign, node.body]
            }
        } ;

        for (var b=0; b<path.length; b++) {
            if (examine(path[b].parent).isBlockStatement) {
                path[b].parent[path[b].field].splice(path[b].index,0,declVars,pushValues) ;
                break ;
            }
        }
        coerce(node,loop) ;
