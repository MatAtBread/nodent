    ...
    var c = {
    	write:function(node) {
            ...
            for (var i = 0; i < parts.length; i++) {
                ...
                if (map && node && node.loc && node.loc.start) {
                  var startOfLine = origLines[node.loc.start.line-1].substring(0,node.loc.start.column) ;
                  startOfLine = startOfLine && startOfLine.match(/^\s*$/);
                	map.addMapping({
                		source: options.map.file,
                		original: { line: node.loc.start.line, column: startOfLine?0:node.loc.start.column+parts[i].length },
