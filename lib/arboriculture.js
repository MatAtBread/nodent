                ...
                if (node.generator) {
                    var body = parser.treeWalker(node.body, function (node, descend, path) {
                        ...
                        if (node.type === 'YieldExpression' && node.delegate) {
                            var yieldStar = parser.part("function*_(){for (var $i of $g) yield $i}",{g:node.argument,i:ident(generateSymbol("i"))}) ;
                            path[0].replace(yieldStar.body[0].body.body[0]) ;
                        } else if (node.type === 'AwaitExpression') {
                            node.type = 'YieldExpression' ;
//                            path[0].replace(parser.part("yield (new (Function.$asyncbind.yieldawait)($arg))",
//                                {arg:node.argument,asyncbind: genIdent.asyncbind}).expr) ;
                        }
                    }) ;
                    body = body.body ;
