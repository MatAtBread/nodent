...
myfn("ok") ;
	...
	function mapTryCatch(node, path, down) {
		if (node.type==='TryStatement' && containsAwait(node) && !node.$mapped) {
			...
			if (node.finalizer) { 
				if (contains(node,function(node){
					return node.type==='ReturnStatement' || node.type==='ThrowStatement'
				})) 
					logger(where(node)+"'finally' keyword does not asynchronize 'return' or 'throw' within try..catch..finally blocks") ;

				var finalizer = makeContinuation((node.$label+"Finally"),cloneNode(node.finalizer.body)) ;
	...
	function labelTryCatch(ast) {
		parser.treeWalker(ast,function(node, descend,path){
			if (node.type==='TryStatement') {
				...
				node.$label = generateSymbol("Try")+"_" ;
				...
				setExit(node,{$error:node.$label+"Catch",$return:parent.$return}) ;
