  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    function transformForIn(node, path) {
        ...
        if (path[0].parent.type==='LabeledStatement') {
          label = path[0].parent.label ;
          loop = {type:'LabeledStatement',label:label,body:loop};
        }
        path[label?1:0].replace([init,node,loop]) ;
    }
    ...
    function mapLoops(node, path, down) {
        ...
        if (!node.$mapped && examine(node).isLoop && containsAwait(node)) {
          ...
          if (opts.noRuntime) {
            invokeIterate = inlineTrampoline(exit) ;
          } else {
            invokeIterate = {
              "type": "ReturnStatement",
              "argument": {
                ...
                "callee": {
                  ...
                  "arguments": [{"type": "ThisExpression"},idContinuation,idStep,exit]
                },
                "arguments": [idIter]
              }
            };
        ...
        function inlineTrampoline(exit) {
            return {
                "type": "ReturnStatement",
                "argument": {
                    "type": "CallExpression",
