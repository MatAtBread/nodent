...
function examine(node) {
    if (!node) 
        return {};
    return {
        getScope: node.type === 'FunctionDeclaration' || node.type === 'FunctionExpression' || node.type === 'Function' || node.type === 'ObjectMethod' || node.type === 'ClassMethod' || node.type === 'ArrowFunctionExpression' && node.body.type === 'BlockStatement' ? node.body.body : node.type === 'Program' ? node.body : null,
        isScope: node.type === 'FunctionDeclaration' || node.type === 'FunctionExpression' || node.type === 'Function' || node.type === 'Program' || node.type === 'ObjectMethod' || node.type === 'ClassMethod' || node.type === 'ArrowFunctionExpression' && node.body.type === 'BlockStatement',
        ...
        isDirective: node.type === 'ExpressionStatement' && (node.expression.type === 'StringLiteral' || node.expression.type === 'Literal' && typeof node.expression.value === 'string'),
        ...
        isES6: node.type === 'ArrowFunctionExpression' 
            || node.type === 'ForOfStatement' 
            || (node.type === 'VariableDeclaration' && node.kind !== 'var')
            || ((node.type === 'FunctionDeclaration' || node.type === 'FunctionExpression') && node.generator === 'true')
...
function asynchronize(pr, __sourceMapping, opts, logger) {
    ...
    function mapCondOp(ast, state) {
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            ...
            if (node.type === 'ConditionalExpression' && (containsAwait(node.alternate) || containsAwait(node.consequent))) {
                ...
                var callBack = {
                    type: 'BlockStatement',
                    body: body.splice(i, body.length - i)
                };
  ...
  function test(x) {
    ...
    function cleanCode(ast) {
        ...
        // Hoist generated FunctionDeclarations within ES5 Strict functions
        var allStrict = ast.type==='Program' && (ast.sourceType === 'module' || isStrict(ast.body)) ;
        ...
        parser.treeWalker(ast, function (node, descend, path) {
            descend();
            if ((node.type==='Program' || node.type==='FunctionDeclaration' || node.type==='FunctionExpression') && (allStrict || isStrict(node.body))) {
                ...
                var functions = scopedNodes(node,function(n,path){
                    return n.type==='FunctionDeclaration' && path[0].parent !== functionScope ; 
                }) ;
