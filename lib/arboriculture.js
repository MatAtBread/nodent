...
function asynchronize(pr, __sourceMapping, opts, logger) {
    ...
    function asyncTransforms(ast, awaitFlag) {
        ...
        (true || opts.promises?Nothing:asyncLoops_1)(ast);
        // Handle the various JS control flow keywords by splitting into continuations that could
        // be invoked asynchronously
        mapLogicalOp(ast);
        mapCondOp(ast);
        walkDown(ast, [mapTryCatch,(true || opts.promises?mapLoops:Nothing),mapIfStmt,mapSwitch,mapBlock]);
        ...
        if (!node.$mapped && examine(node).isLoop && containsAwait(node)) {
          ...
          if ('index' in ref) {
            ...
            continuation = makeContinuation(idContinuation, deferredCode) ;
              ...
              continuation = makeContinuation(idContinuation, []) ;
          }
              ...
              if (n.type === 'BreakStatement' || n.type === 'ContinueStatement') {
                  if (n.label) {
                      ...
                      for (var l=0; l<=path.length; l++) {
                          if (!path[l])
                              throw new Error(where(n) + "Unknown loop label: "+n.label);

                          if (examine(path[l].self).isLoop) {
                              if (path[l].parent.type === 'LabeledStatement' && path[l].parent.label.name === n.label.name) {
                                  loops.push(literal(n.type === 'ContinueStatement'?1:0)) ;
                                  subpath[0].replace({
                                      type: 'ReturnStatement',
                                      argument: {
                                          "type": "ArrayExpression",
                      ...
                      if (n.type === 'BreakStatement')
                          subpath[0].replace(returnExit(0)) ;
                      else // continue;
                          subpath[0].replace(returnExit(1)) ;
                  }
                                              ...
                                              "body": [{
                                                  ...
                                                  "alternate": {
                                                      ...
                                                      "consequent":{
                                                          ...
                                                          alternate:{
                                                              type:'ReturnStatement',
                                                              argument:thisCall(idContinuation/*,idBounce*/)
                                                          }
                                                      }
                                                  }
                                              }]
