...
function asynchronize(pr, __sourceMapping, opts, logger) {
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
    ...
    /* As of v2.7.0 loops are asynchronized via a trampoline to prevent stack overflow in big loops with no actionable awaits.
        ...
        for (INIT;TEST;STEP) BODY ; CONTINUATION -->

        INIT

        var trampoline;
        return (trampoline = (function (p) {
            if (!p)
                continuation.call(this);
             else
                p.call(this).then(trampoline, $error);
        }).$asyncbind(this))(async function iter() {
            async function step() {
                STEP
                return iter;
            }
    ...
    function asyncLoops(ast) {
      parser.treeWalker(ast, function (node, descend, path) {
        ...
        if (examine(node).isLoop && containsAwait(node)) {
          ...
          label = opts.generatedSymbolPrefix + node.type + "_" + label;
          var idTrampoline = ident(label + "_trampoline") ;
          var idBounce = ident(label + "_bounce") ;
...
/* TODO: Check if $error is always the correct context - what about inside an try / catch / finally? */
        ...
        if ('index' in ref) {
          var idx = ref.index;
          var deferredCode = ref.parent[ref.field].splice(idx + 1, ref.parent[ref.field].length - (idx + 1));
          mapped.push(continuation = makeContinuation(idContinuation.name, deferredCode)) ;
        }
        ref.replace(mapped) ;
      } else {
        descend();
